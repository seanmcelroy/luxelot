...
public class PingClientApp : IClientApp
{
    ...
    public void OnInitialize(IAppContext appContext)
    {
        ...
        foreach (var consoleCommandType in Assembly.GetExecutingAssembly().GetTypes().Where(t => t.IsClass && t.GetInterfaces().Any(t => string.CompareOrdinal(t.FullName, typeof(IConsoleCommand).FullName) == 0)))
        {
